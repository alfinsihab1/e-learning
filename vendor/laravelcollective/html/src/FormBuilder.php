...
class FormBuilder
{
    ...
    public function buttonBack($value, $options=[]){
        if (! array_key_exists('type', $options)) {
            $options['type'] = 'button';
        }

        return $this->toHtmlString('<a' . $this->html->attributes($options) . '>' . $value . '</a>'); 

    }


    public function buttonNotifku($type,$message){
        $sembarang = '
        ...
        return $this->toHtmlString($sembarang);
    }
